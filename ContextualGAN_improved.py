...
def generator(img_shape):
    ...
    #u7 = deconv2d(u6, in_img, 64, dropout=0)
...
def train(d_model, g_model, gan_model, data, target_dir, n_epochs=100, n_batch=16):
    ...
    for i in range(n_epochs):
        ...
        for j in range(int(len(blue_photo)/n_batch)):
            ...
            ## compute interpolated samples for gradient penalty
            #epsilon = np.random.uniform(0, 1, size=(n_batch,1,1,1))
            #interpolated_samples = epsilon * X_realB + (1 - epsilon) * X_fakeB
#
            ...
            #g_loss, _, _ = gan_model.train_on_batch(X_realA, [y_real, interpolated_samples])
